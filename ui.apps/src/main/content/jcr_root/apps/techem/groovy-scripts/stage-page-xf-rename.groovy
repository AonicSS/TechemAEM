...
def sqlQuery = "select [jcr:path], * from [nt:unstructured] as a where [cq:template] = '/conf/techem/settings/wcm/templates/stage-content-page' and isdescendantnode(a, '/content/techem')";
...
queryResults.nodes.each {
    node ->
        ...
        // FOOTER EXP FRAGMENT
        if (node.hasNode("experiencefragment_1433873681")) {
            def nodeItr =node.getNodes();
            def navId="";
            def navName="";
            nodeItr.each{
                navTabsNode ->
                    // rename exp footer
                    if (navTabsNode.getName().equals("experiencefragment_1433873681")) {
                        println "Node at path = " + nodePath + " needs update (FOOTER)"
                        navTabsNode.set("fragmentVariationPath","");
                        def srcPath = nodePath + "/" + "experiencefragment_1433873681";
